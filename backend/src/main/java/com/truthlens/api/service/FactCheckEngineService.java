...
public class FactCheckEngineService {
    ...
    public ClaimVerificationResponse verifyClaim(ClaimVerificationRequest request) {
        ...
        // 3. Decompose Claim into Atomic Propositions
        List<DecomposedClaim> decomposedSubClaims = claimDecompositionService.decompose(contentToAnalyze);

        // 4. Run NLP Pipeline
        NlpAnalysisResponse nlpResults = nlpPipelineService.processText(contentToAnalyze);

        // 5. Match against Fact-Checking Corpus using TF-IDF & Cosine Similarity
        MatchResult corpusMatch = factCheckingCorpus.matchClaimAgainstCorpus(contentToAnalyze);

        // 6. Query External Knowledge & Live News Wires for Origin & Cross-Referencing
        ...
        // 12. Build Claim Origin & Provenance Discovery
        ...
        // 13. Build Structured Explainability Profile
        ExplainabilityProfile explainability = buildExplainabilityProfile(contentToAnalyze, score, verdict, confidence, sources, evidenceClusters, externalFact, corpusMatch);

        // 14. Decouple Content Characteristics / Sensationalism
    ...
    private void evaluateSubClaims(List<DecomposedClaim> subClaims, MatchResult corpusMatch, Optional<ExternalFactCheckService.ExternalFactResult> externalFact) {
        ...
        for (DecomposedClaim sub : subClaims) {
            ...
            if (isSubContradicted) {
                ...
                sub.setClaimScore(18);
                sub.setClaimVerdict("REFUTED");
                sub.setStance("REFUTED");
                sub.setEvidenceSummary("Contradicted by verified records: " + reason);
    ...
    private int calculateGenuinenessScore(String text, NlpAnalysisResponse nlp,
        ...
        if (verifiedSim >= 0.45 && isVerifiedCompatible && verifiedSim > debunkedSim) {
            ...
            return Math.max(78, Math.min(98, baseVerifiedScore));
        }
        ...
        // 6. General Unverified / Breaking Claim (Zero contradictory evidence & zero confirming evidence)
        return 48; // Insufficient evidence baseline
    }

    private String determineVerdict(int score, Optional<ExternalFactCheckService.ExternalFactResult> externalFact, MatchResult match) {
        if (score >= 85) return "VERIFIED GENUINE";
        if (score >= 70) return "MOSTLY GENUINE";
        if (score <= 35) return "FABRICATED / FAKE";

        // Epistemic distinction: If no sources confirm and no sources contradict, return INSUFFICIENT_EVIDENCE
    ...
    private String getVerdictBadgeColor(String verdict, int score) {
        ...
        if ("MIXED / UNVERIFIED".equals(verdict)) return "#F59E0B"; // Amber Yellow
        return "#EF4444"; // Crimson Red
    }

    private String determineConfidence(int score, MatchResult match, Optional<ExternalFactCheckService.ExternalFactResult> externalFact) {
        if (externalFact.isPresent() && externalFact.get().getCrossReferencedSources().size() >= 2) return "HIGH";
        if (match.getTopSimilarity() >= 0.65) return "HIGH";
        if (externalFact.isPresent()) return "MEDIUM";
        if (match.getTopSimilarity() >= 0.40) return "MEDIUM";
        return "LOW";
    }
    ...
    private ExplainabilityProfile buildExplainabilityProfile(String text, int score, String verdict, String confidence,
        ...
        boolean isContradicted = (externalFact.isPresent() && externalFact.get().isContradiction()) ||
                                 (match.getBestVerifiedEntry() != null && checkContradictionSafely(text, match.getBestVerifiedEntry().getText()).isContradicted());
        ...
        if (externalFact.isPresent()) {
            ...
            if (res.isAuthenticCorroboration()) {
                positive.add("Original published dispatch located from " + res.getSourceName() + ".");
                positive.add("Factual entities and core topic aligned with accredited reporting.");
            } else if (res.isContradiction()) {
                warning.add("Contradiction detected against verified report: " + res.getContradictionDetail());
            ...
            warning.add("Emerging claim requires independent corroboration before acceptance.");
        }
    ...
    private List<String> generateKeyReasons(String text, NlpAnalysisResponse nlp, int score,
        ...
        if (externalFact.isPresent()) {
            ...
            if (fact.isContradiction()) {
                reasons.add("Contradicted by verified press reporting from " + fact.getSourceName() + ".");
                if (fact.getContradictionDetail() != null) {
                    reasons.add(fact.getContradictionDetail());
                }
                reasons.add("Key factual metrics (casualties/outcome) directly conflict with verified dispatches.");
                return reasons;
        ...
        if (match.getBestVerifiedEntry() != null && match.getVerifiedSimilarity() >= 0.35) {
            ...
            if (corpusContradiction != null && corpusContradiction.isContradicted()) {
                ...
                reasons.add("Falsified claim alters verified historical casualty records.");
                return reasons;
            }
        }

        if (image != null && image.getManipulationProbability() > 70) {
            reasons.add("Visual Error Level Analysis indicates pixel tampering (" + String.format("%.1f", image.getManipulationProbability()) + "%).");
    ...
    private String buildRationaleText(String text, int score, String verdict, NlpAnalysisResponse nlp,
        ...
        Optional<String> demographicAnomaly = checkDemographicAnomaly(text);
        if (demographicAnomaly.isPresent()) {
            return "TruthLens has evaluated this submission as FABRICATED / FAKE. " + demographicAnomaly.get();
        }

        if (externalFact.isPresent()) {
            ExternalFactCheckService.ExternalFactResult fact = externalFact.get();
            if (fact.isContradiction()) {
                return "TruthLens has evaluated this submission as FABRICATED / FAKE. Contradicted by verified press reporting from " +
                        fact.getSourceName() + ": \"" + fact.getTopic() + "\". " + fact.getContradictionDetail();
            } else if (fact.isAuthenticCorroboration()) {
        ...
        if (match.getBestVerifiedEntry() != null && match.getVerifiedSimilarity() >= 0.35) {
            ExternalFactCheckService.ContradictionCheck corpusContradiction = checkContradictionSafely(text, match.getBestVerifiedEntry().getText());
            if (corpusContradiction != null && corpusContradiction.isContradicted()) {
                return "TruthLens has evaluated this submission as FABRICATED / FAKE. Contradicted by verified documentary records from " +
    ...
    private ClaimOriginDiscovery buildClaimOriginDiscovery(String text, int score, String verdict,
        ...
        if (corpusMatch.getBestDebunkedEntry() != null && corpusMatch.getDebunkedSimilarity() >= 0.45) {
            ...
            return ClaimOriginDiscovery.builder()
                    ...
                    .provenanceStatus("PRIMARY_SOURCE_FOUND")
                    .evidenceTier("LEVEL_3_FACTCHECK")
        ...
        if (corpusMatch.getBestVerifiedEntry() != null && corpusMatch.getVerifiedSimilarity() >= 0.45) {
            ...
            return ClaimOriginDiscovery.builder()
                    ...
                    .provenanceStatus(isCorpusContradicted ? "ORIGINAL_REPORT_FOUND" : "PRIMARY_SOURCE_FOUND")
                    .evidenceTier("LEVEL_2_SECONDARY")
